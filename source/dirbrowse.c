    ...
    switch (config_sort_by) {
        ...
        case 2:
            if (isDir[0] == isDir[1]) // Size newest
                return sizeA > sizeB ? -1 : sizeA < sizeB ? 1
                                                          : 0;
            else
                return isDir[1] - isDir[0];
            break;
        ...
        case 3:
            if (isDir[0] == isDir[1]) // Size oldest
                return sizeB > sizeA ? -1 : sizeB < sizeA ? 1
                                                          : 0;
            else
    ...
    for (; file != NULL; file = file->next) {
        ...
        if (position < FILES_PER_PAGE || i > (position - FILES_PER_PAGE)) {
            ...
            if (strcmp(multi_select_dir, cwd) == 0) {
                multi_select[i] == true ? SDL_DrawImage(RENDERER, config_dark_theme ? icon_check_dark : icon_check, 20, 156 + (73 * printed)) : SDL_DrawImage(RENDERER, config_dark_theme ? icon_uncheck_dark : icon_uncheck, 20, 156 + (73 * printed));
            } else
            ...
            else if ((strncasecmp(FS_GetFileExt(file->name), "nro", 3) == 0) || (strncasecmp(FS_GetFileExt(file->name), "elf", 3) == 0) || (strncasecmp(FS_GetFileExt(file->name), "bin", 3) == 0))
                SDL_DrawImageScale(RENDERER, icon_app, 80, 141 + (73 * printed), 72, 72);
            else if ((strncasecmp(FS_GetFileExt(file->name), "zip", 3) == 0) || (strncasecmp(FS_GetFileExt(file->name), "tar", 3) == 0) || (strncasecmp(FS_GetFileExt(file->name), "lz4", 3) == 0))
                SDL_DrawImageScale(RENDERER, icon_archive, 80, 141 + (73 * printed), 72, 72);
            else if ((strncasecmp(FS_GetFileExt(file->name), "mp3", 3) == 0) || (strncasecmp(FS_GetFileExt(file->name), "ogg", 3) == 0) || (strncasecmp(FS_GetFileExt(file->name), "wav", 3) == 0) || (strncasecmp(FS_GetFileExt(file->name), "mod", 3) == 0) || (strncasecmp(FS_GetFileExt(file->name), "flac", 4) == 0) || (strncasecmp(FS_GetFileExt(file->name), "midi", 4) == 0) || (strncasecmp(FS_GetFileExt(file->name), "mid", 3) == 0))
                SDL_DrawImageScale(RENDERER, icon_audio, 80, 141 + (73 * printed), 72, 72);
            else if ((strncasecmp(FS_GetFileExt(file->name), "png", 3) == 0) || (strncasecmp(FS_GetFileExt(file->name), "jpg", 3) == 0) || (strncasecmp(FS_GetFileExt(file->name), "bmp", 3) == 0) || (strncasecmp(FS_GetFileExt(file->name), "gif", 3) == 0))
                SDL_DrawImageScale(RENDERER, icon_image, 80, 141 + (73 * printed), 72, 72);
            else if ((strncasecmp(FS_GetFileExt(file->name), "txt", 3) == 0) || (strncasecmp(FS_GetFileExt(file->name), "lua", 3) == 0) || (strncasecmp(FS_GetFileExt(file->name), "cfg", 3) == 0))
