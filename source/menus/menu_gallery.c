...
static void Gallery_GetImageList(void) {
    ...
    if (dir != NULL) {
        while ((entries = readdir(dir)) != NULL) {
            if ((strncasecmp(FS_GetFileExt(entries->d_name), "png", 3) == 0) || (strncasecmp(FS_GetFileExt(entries->d_name), "jpg", 3) == 0) || (strncasecmp(FS_GetFileExt(entries->d_name), "bmp", 3) == 0) || (strncasecmp(FS_GetFileExt(entries->d_name), "gif", 3) == 0)) {
...
static void Gallery_DrawImage(int x, int y, int w, int h, float zoom_factor) {
    ...
    position.x = x;
    position.y = y;
    position.w = w * zoom_factor;
    position.h = h * zoom_factor;
